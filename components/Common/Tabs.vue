    ...
    <div id="MainTabs" class="sticky top-0 bg-white mr-2">
      ...
      <span v-else-if="currentMenu === 3"> <!-- FORECAST TABS -->
        <div class="@md:bg" style="margin-left: 0px; overflow: hidden">
          ...
          <div v-if="calibrationJobId" data-tab="4" class="tabs prevent-select" v-on:click="tabClicked" aria-label="Status/Run tab"
            title="Status/Run Tab">
            Status/Run
            <div :class="tabNotCompleted ? 'errorDot' : 'noErrorDot'"></div>
          </div>
...
const { forecastJobId, forecastJobStatus } = storeToRefs(useForecastStore());
